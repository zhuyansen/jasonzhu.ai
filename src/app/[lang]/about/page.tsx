...
export async function generateMetadata({
  ...
  return {
    ...
    alternates: {
      canonical: `https://jasonzhu.ai/${lang}/about`,
      languages: { zh: "https://jasonzhu.ai/zh/about", en: "https://jasonzhu.ai/en/about", "x-default": "https://jasonzhu.ai/zh/about" },
    },
  };
}
...
export default async function AboutPage({
  ...
  const timeline = [
    {
      period: dict.about.now,
      title: dict.about.nowTitle,
      desc: dict.about.nowDesc,
    },
    {
  ...
  const socials = [
    ...
    },
    {
      name: "GitHub",
      handle: "zhuyansen",
      url: "https://github.com/zhuyansen" as string | null,
      icon: (
        <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 sm:px-6 py-12">
      ...
      <section className="mb-12">
        <h2 className="text-xl font-bold text-gray-900 mb-4">{dict.about.whatIDo}</h2>
        <div className="space-y-4">
          <div className="flex gap-3">
            <span className="text-xl">✍️</span>
            <div>
              <h3 className="font-semibold text-gray-900">{dict.about.contentCreation}</h3>
              <p className="text-sm text-gray-500">
                {dict.about.contentCreationDesc}
              </p>
            </div>
          </div>
          <div className="flex gap-3">
            ...
            <div>
              <h3 className="font-semibold text-gray-900">{dict.about.training}</h3>
              <p className="text-sm text-gray-500">
                {dict.about.trainingDesc}
              </p>
            </div>
          </div>
          <div className="flex gap-3">
            <span className="text-xl">📡</span>
            <div>
              <h3 className="font-semibold text-gray-900">{dict.about.mcn}</h3>
              <p className="text-sm text-gray-500">
                {dict.about.mcnDesc}
              </p>
            </div>
          ...
          <div className="flex gap-3">
            <span className="text-xl">🛠️</span>
            <div>
              <h3 className="font-semibold text-gray-900">{dict.about.dev}</h3>
              <p className="text-sm text-gray-500">
                {dict.about.devDesc}
              </p>
            </div>
      ...
      {/* Projects */}
      <section className="mb-12">
        <h2 className="text-xl font-bold text-gray-900 mb-4">{dict.about.myProjects}</h2>
      ...
      {/* Timeline */}
      <section className="mb-12">
        <h2 className="text-xl font-bold text-gray-900 mb-4">{dict.about.career}</h2>
      ...
      {/* Social Links */}
      <section>
        <h2 className="text-xl font-bold text-gray-900 mb-4">{dict.about.findMe}</h2>
        <div className="space-y-3">
          {socials.map((social) => {
            const content = (
              <>
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-400">
                    {social.handle}
                    {!social.url && <span className="ml-1 text-gray-300">{dict.about.wechatNote}</span>}
                  </p>
                </div>
              </>
            );
